			...
			tbar:{height:27, items:[{
				...
				handler:load,
					...
					xtype:'label', text:'Подразделение',
						style: { fontWeight:'bold',marginLeft:10, marginRight:3, color:'navy',},
				}, {
					xtype:'textfield', id:'sea_Dep',
						enableKeyEvents:true, emptyText:'поиск', minLength: 3, width: 150,
						mode: 'local',
							allowBlank:true, forceSelection:true, editable:false,
							selectOnFocus:true, autoSelect:false, triggerAction: 'all',
						store: new Ext.data.JsonStore({}),
							valueField: 'Dep_Name',	displayField: 'NAME',
						listeners:{select:function(){
							//document._dep_id=xg('sea_Dep').getValue();
							document.tb.hd.DEP_NAME=xg('sea_Dep').getValue();
							//document.modifyed++; showBtns();
						}
...
function loadDocData(resp)
{
	...
	xg('sea_Dep').setRawValue(resp.hd.seaDep);
	console.log(resp.hd.seaDep);
	...
	var se_conf = {
		...
		local_data:document.tb.rows,
			search_fields:'PNAME,VNAME,DEP_NAME',
...
function mkRepoShow()
{
	...
	function(resp){
		document.tb.D=resp;
	//xg('dbgD').getStore().loadData(resp);
		var se_conf = {
			searcher:'eSeaD', dbg:'dbgD',
				local_data:resp,
			search_fields:'NAME',
				key_fields:'DEP_ID',
			editor:function(se,fn,auto,val){
				se.selected.data.SELECTED=
					se.selected.data.SELECTED?0:1;
				se.save_row();
			},
			/*data_change:function(data){
				xg('FMON.rowscount').setText(xg('dbg').getStore().getCount()); },*/
		}; document.se.D = new M2_EXT_SEARCHER_GRID(se_conf);
	});
}
